...
public class PricePlanService {
    ...
    public Map<String, BigDecimal> getCostForPlanId(List<ElectricityReading> currentReadings,String planId) throws JOIEnergyException{
    	
    	Map<String , BigDecimal> costOfUsageMap = new HashMap<String, BigDecimal>();
    	
     Optional<PricePlan> pricePlan = pricePlans.stream().filter(plan->plan.getPlanName().equals(planId)).findFirst();
    
    
    if(pricePlan.isPresent()) {
    BigDecimal temp = 	calculateCost(currentReadings, pricePlan.get());
    	 costOfUsageMap.put(planId, temp);
    	 
    	 return costOfUsageMap;
    }
    
    else {
    	throw new JOIEnergyException("Price Plan Not Found");
    }
